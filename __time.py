...
#
time()            # 1529239510.737
localtime(SECS)   # 本地秒数转换为本地时间元组对象
gmtime(SECS)      # 本地秒数转换为UTC时间元组对象
...
strftime(ISO_FORMAT, TUP)        # 元组到指定格式， '2018-06-17 20:45:10'
strptime(ISO_TM, ISO_FORMAT)     # 指定格式到元组
...
#
# Datetime对象减10分钟
DATE_TIME = datetime.fromtimestamp((mktime(DATE_TIME.timetuple())-600))  # datetime.datetime(2018, 6, 17, 20, 35, 10)

# Datetime对象减10分钟
ctup = strptime(DATE_TIME.ctime(), C_FORMAT)
DATE_TIME = datetime.fromtimestamp(mktime(ctup)-600)        # datetime.datetime(2018, 6, 17, 20, 25, 10)